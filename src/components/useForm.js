...
import { ERROR, get_prop_values, is_object, is_required, VALUE } from './utils';
...
function useForm(stateSchema = {}, stateValidatorSchema = {}, submitFormCallback) {
  ...
  const setFieldError = ({ name, error }) => setErrors((prevState) => ({ ...prevState, [name]: error }));
  ...
  const setInitialErrorState = useCallback(() => {
    Object.keys(errors).map((name) => setFieldError({ name, error: validateField(name, values[name]) }));
